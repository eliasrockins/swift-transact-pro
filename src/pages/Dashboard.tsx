...
export default function Dashboard() {
  ...
  // FUNÇÃO PARA CALCULAR 3 HORAS À FRENTE
  const getTempoExpiracao = () => {
    const agora = new Date();
    agora.setHours(agora.getHours() + 3); // Soma 3 horas
    ...
    <div className="flex min-h-screen bg-[#f8f9fa] font-sans relative">
      ...
      <main className="flex-1 p-4 md:p-8 overflow-y-auto pb-24 md:pb-8 relative">
        ...
        <header className="mb-8 flex flex-col items-start gap-1">
          <img src={logo} alt="Ck Soluções" className="hidden md:block h-14 w-auto mb-4 object-contain drop-shadow-sm" />
        ...
        {abaAtiva === 'pedidos' && (
          <div className="space-y-6">
            <h3 className="text-2xl font-bold text-[#1f2937] mb-6">Histórico de Pedidos</h3>
            {pedidos.length === 0 ? (
                ...
                {pedidos.map((p) => (
                  <div key={p.id} className="bg-white p-5 rounded-xl border border-gray-100 shadow-sm space-y-3">
                    <div className="flex justify-between items-center">
                      <h4 className="font-bold text-gray-900 text-sm">{p.produto}</h4>
                      {p.status === 'pago' ? (
                    ...
                    <div className="pt-1">
                      {p.status === 'pago' ? (
                        ...
                        <button onClick={() => abrirPagamento(p)} className="bg-[#4ade80] hover:bg-[#22c55e] text-white px-6 py-3 rounded-lg font-bold text-sm shadow-sm transition-colors w-full active:scale-95">
                          Pagar
                        </button>
                      )}
                    </div>
      ...
      {/* MODAL DE PAGAMENTO PIX ATUALIZADO */}
      {pagamentoAberto && (
        <div className="fixed inset-0 bg-gray-50/90 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-[24px] w-full max-w-md p-8 shadow-2xl relative border border-gray-100 animate-in zoom-in duration-200">
            ...
            <textarea readOnly value={pagamentoAberto.pix_copia_cola} className="w-full p-4 border border-gray-200 rounded-xl font-mono text-[11px] text-gray-500 h-28 mb-4 bg-gray-50 resize-none outline-none custom-scrollbar" />
            <button onClick={() => copiarPix(pagamentoAberto.pix_copia_cola, pagamentoAberto.id)} className={`w-full py-4 rounded-xl font-bold text-white shadow-lg transition-all flex items-center justify-center gap-2 text-lg ${copiou === pagamentoAberto.id ? 'bg-blue-600 shadow-blue-200' : 'bg-[#4ade80] hover:bg-[#22c55e] shadow-green-200 active:scale-95'}`}>
              {copiou === pagamentoAberto.id ? <Check size={24} /> : <Copy size={24} />}
              {copiou === pagamentoAberto.id ? 'Código Copiado!' : 'Copiar Código Pix'}
            </button>

            {/* BOX DE EXPIRAÇÃO ATUALIZADO PARA 3 HORAS */}
            <div className="mt-4 py-3 bg-gray-50 rounded-xl flex justify-center items-center text-gray-500 font-mono text-sm font-bold border border-gray-200">
              <Clock size={18} className="mr-2" /> Expira em: {getTempoExpiracao()}
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-3xl w-full max-w-md p-8 relative shadow-2xl animate-in zoom-in duration-200">
            ...
            <div className="space-y-5">
              <div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Nome do Titular da Conta</label><input readOnly value={`${perfil?.nome} ${perfil?.sobrenome}`} className="w-full p-4 bg-gray-50 border border-gray-200 rounded-xl text-gray-600 font-bold outline-none" /></div>
              <div><label className="block text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2">Código de validação</label><input placeholder="Digite o código aqui..." className="w-full p-4 border border-gray-300 rounded-xl outline-none focus:ring-2 focus:ring-blue-500 text-gray-900 font-medium placeholder:text-gray-400" /></div>
              <button className="w-full bg-[#28a745] hover:bg-[#218838] text-white py-4 rounded-xl font-black shadow-lg shadow-green-100 transition-all active:scale-95 mt-4 uppercase tracking-widest text-xs">Enviar Solicitação</button>
            </div>
          </div>
        </div>
      )}
